...
def main():
	print(BANNER)
	print(MESSAGE)
	print("\n\nThis is a very old meal, I cannot recognize its taste anymore:", enc_flag) #enc(flag)
	...
	try:
		...
		if choice == "1":
			...
			beyet = jammer_elbeyet(inp)  #decrypt(inp)

			if check_leak(beyet):
				print("No No Idi*t! You burned l3ché!")
			else:
				print("Mekla Mjamra: ", beyet.hex())  #decrypt(inp)
